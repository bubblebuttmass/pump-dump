...
// users_display_name_lower_unique (migration 0027) is the source of truth --
// this is just a fast, non-authoritative pre-check so the UI can tell someone
// a name is taken before they even hit save, not a replacement for it.
...
// users_display_name_lower_unique" is meaningless to a user -- this is the
// one place that turns it into copy someone can act on. Also the backstop
// for the race the client-side isUsernameTaken check can't catch (two
// people submitting the same name at once).
export function toProfileSaveError(error: any): Error {
  if (error?.code === POSTGRES_UNIQUE_VIOLATION) return new Error('That username is already taken.');
  return error;
}
