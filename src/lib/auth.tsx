...
interface AuthContextValue {
  ...
  // picking a display name + photo. Root routing (see app/index.tsx) uses
  // this to send an interrupted signup back to onboarding instead of the
  // feed.
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const { data: listener } = supabase.auth.onAuthStateChange((_event, newSession) => {
      ...
      setSession((prev) => {
        const unchanged = prev?.user?.id === newSession?.user?.id;
        if (!unchanged) setOnboardingComplete(null);
        return unchanged ? prev : newSession;
      });
    });
  ...
  // Separate from the session effect above so a token refresh (same user,
  // reference held) never re-triggers this -- only an actual user change
  // (including sign-out, which clears it back to null) should.
  useEffect(() => {
    if (!session?.user) {
